...
int Physic::fall(Point p_)//オブジェクトの座標を引数にして実際にどれだけ落下したかを返す
{
	...
	while (prevel != 0) {                                                //めり込まない移動処理
		...
		if (ct->gts->map->get_floar_bottom(hit) == 1) {                        //マップとの当たり判定
			velocity = velocity - prevel -1;                                //直前のprevelの計算をなかったことにし、マップとの距離を求めvelocityに代入
			break;
		}
	}                               
	return velocity;
...
int Physic::Movement_Y(Point p_, int velocity_)
{
	...
	int prevel = velocity_;                                                            //計算用かつ、velocityを不変のものであるために仮の値を用意
	while (prevel != 0) {                                                             //移動方向は+方向かマイナス方向かの判定し、１ドットずつめり込まないかどうかの判定を行う
